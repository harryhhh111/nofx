...
var promptTimeframeOrder = map[string]int{
	"1m":  10,
	"3m":  20,
	"5m":  30,
	"15m": 40,
...
func orderedTimeframesForPrompt(timeframes []string) []string {
	seen := make(map[string]struct{}, len(timeframes))
	ordered := make([]string, 0, len(timeframes))

	for _, tf := range timeframes {
		norm := strings.TrimSpace(strings.ToLower(tf))
		if norm == "" {
			continue
		}
	...
	sort.SliceStable(ordered, func(i, j int) bool {
		pi, iok := promptTimeframeOrder[ordered[i]]
		pj, jok := promptTimeframeOrder[ordered[j]]
		switch {
		case iok && jok:
			return pi < pj
		case iok:
...
func joinTimeframesForPrompt(timeframes []string) string {
	return strings.Join(orderedTimeframesForPrompt(timeframes), ", ")
}
...
func formatContextData(ctx *Context, lang Language) string {
	...
	if len(ctx.Timeframes) > 0 {
		if lang == LangChinese {
			sb.WriteString(fmt.Sprintf("## 分析K线周期\n\n必须逐个分析以下已配置周期：`%s`\n\n", joinTimeframesForPrompt(ctx.Timeframes)))
		} else {
			sb.WriteString(fmt.Sprintf("## Analysis Timeframes\n\nYou MUST analyze each configured timeframe for every symbol: `%s`\n\n", joinTimeframesForPrompt(ctx.Timeframes)))
		}
	}
...
func formatKlineDataEN(symbol string, tfData map[string]*market.TimeframeSeriesData, timeframes []string) string {
	...
	for _, tf := range orderedTimeframesForPrompt(timeframes) {
		if data, ok := tfData[tf]; ok && len(data.Klines) > 0 {
