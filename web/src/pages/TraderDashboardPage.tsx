...
function formatBBMACDSubtitle(stats?: BBMACDAccuracyStats): string {
  ...
  const rawAccuracy =
    stats.breakout_overall.resolved > 0
      ? `${stats.breakout_overall.accuracy.toFixed(1)}%`
      : '--'
  const threshold = stats.effective_threshold_pct?.toFixed(1) ?? '0.3'
  const label =
    stats.breakout_effective.resolved > 0 ? 'breakout eff' : 'breakout raw'
...
export function TraderDashboardPage({
  ...
  const [bbmacdDraft, setBBMACDDraft] =
    useState<BBMACDConfig>(defaultBBMACDConfig)
  ...
  const saveBBMACDConfig = async () => {
    ...
    try {
      ...
      notify.error(
        error instanceof Error ? error.message : 'BB MACD 参数保存失败'
      )
    } finally {
  ...
  const protectiveSourceLabel = (source?: string) => {
    ...
    if (source.includes('market_structure.resistance'))
  ...
  const positionExecutionRiskReward = (pos: Position) => {
    ...
    if (!pos.entry_price || !pos.stop_loss_price || !pos.take_profit_price)
      return 0
  ...
  const protectiveDetailTitle = (pos: Position, type: 'sl' | 'tp') => {
    ...
    const timeframe =
      type === 'sl' ? pos.stop_loss_timeframe : pos.take_profit_timeframe
    ...
    const parts = [
      ...
      type === 'tp' &&
      pos.take_profit_candidate_count &&
      pos.take_profit_candidate_count > 0
        ? `Candidates: ${pos.take_profit_candidate_count}`
        : '',
      type === 'tp' &&
      pos.take_profit_min_risk_reward &&
      pos.take_profit_min_risk_reward > 0
        ? `Min R:R: ${pos.take_profit_min_risk_reward.toFixed(2)}`
        : '',
      type === 'tp' &&
      pos.take_profit_min_atr_distance &&
      pos.take_profit_min_atr_distance > 0
        ? `Min distance: ${pos.take_profit_min_atr_distance.toFixed(2)} ATR`
        : '',
      type === 'tp' &&
      pos.take_profit_selected_risk_reward &&
      pos.take_profit_selected_risk_reward > 0
        ? `Selected R:R: ${pos.take_profit_selected_risk_reward.toFixed(2)}`
        : '',
      type === 'tp' &&
      pos.take_profit_selected_atr_distance &&
      pos.take_profit_selected_atr_distance > 0
        ? `Selected distance: ${pos.take_profit_selected_atr_distance.toFixed(2)} ATR`
        : '',
      type === 'tp' && pos.nearest_take_profit && pos.nearest_take_profit > 0
        ? `Nearest target: ${formatPrice(pos.nearest_take_profit)}${pos.nearest_take_profit_risk_reward && pos.nearest_take_profit_risk_reward > 0 ? ` (R:R ${pos.nearest_take_profit_risk_reward.toFixed(2)})` : ''}`
        : '',
      pos.protective_atr && pos.protective_atr > 0
        ? `ATR: ${formatPrice(pos.protective_atr)}${pos.protective_atr_timeframe ? ` (${pos.protective_atr_timeframe})` : ''}`
        : '',
      pos.protective_atr_buffer && pos.protective_atr_buffer > 0
        ? `ATR buffer: ${pos.protective_atr_buffer}x`
        : '',
      pos.protective_risk_reward && pos.protective_risk_reward > 0
        ? `Structural R:R: ${pos.protective_risk_reward.toFixed(2)}`
        : '',
      ...
      <div className="w-full px-4 md:px-8 relative z-10 pt-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          ...
          <StatCard
            ...
            unit={
              bbmacdStats?.breakout_effective?.resolved ||
              bbmacdStats?.breakout_overall?.resolved
                ? '%'
                : undefined
            }
            subtitle={
              bbmacdStatsError
                ? 'ALL | -- samples'
                : formatBBMACDSubtitle(bbmacdStats)
            }
            icon="BB"
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
            <div className="w-full max-w-md rounded-lg border border-white/10 bg-[#101418] p-5 shadow-2xl">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-semibold text-nofx-text-main">
                    BB MACD 参数
                  </h3>
              ...
              <div className="grid grid-cols-2 gap-3">
                {(
                  ...
                  ] as Array<[keyof BBMACDConfig, string]>
                ).map(([key, label]) => (
              ...
              <div className="mt-4 rounded border border-white/10 bg-black/20 p-3 text-xs leading-relaxed text-nofx-text-muted">
                上行趋势可先试 8-21-5；更慢的趋势过滤可试 5-35-5。参数只影响 BB
                MACD 统计，不注入 AI 决策。
              </div>
                    ...
                    <table className="w-full text-xs">
                      <thead className="text-left border-b border-white/5">
                        <tr>
                          ...
                          <th
                            className="px-1 pb-3 font-semibold text-nofx-text-muted whitespace-nowrap text-right hidden xl:table-cell"
                            title="Stop loss"
                          >
                            SL
                          </th>
                              ...
                              <td className="px-1 py-3 font-mono whitespace-nowrap text-right hidden xl:table-cell">
                                ...
                                pos.take_profit_price > 0 ? (
                                  <div title={protectiveDetailTitle(pos, 'tp')}>
                                    <div className="text-nofx-green">
                                      {formatPrice(pos.take_profit_price)}
                                    </div>
                                    <div className="text-[9px] text-nofx-text-muted font-sans">
                                      {protectiveSourceLabel(
                                        pos.take_profit_source
                                      ) || '-'}
                                    </div>
                                  </div>
                                ) : (
                              ...
                              <td colSpan={10} className="px-1 pb-3">
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-2 text-[10px] text-nofx-text-muted">
                                  ...
                                  <div>
                                    ...
                                    <span className="block text-nofx-text-muted">
                                      {protectiveSourceLabel(
                                        pos.stop_loss_source
                                      ) || '-'}
                                    </span>
    ...
    <div className="nofx-glass p-6 animate-slide-in">
      <div className="mb-5 flex items-center justify-between border-b border-white/5 pb-4">
        <div>
          ...
          <p className="mt-1 text-xs text-nofx-text-muted">
            Slippage, fill ratio, and order timing
          </p>
        </div>
      ...
      {failed ? (
        <div className="py-10 text-center text-sm text-nofx-text-muted">
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {records.map((record) => (
                ...
                >
                  <td className="py-2 font-mono font-semibold text-nofx-text-main">
                    {record.symbol}
                  </td>
                  <td className="py-2 text-nofx-text-muted">{record.action}</td>
                  <td
                    className={`py-2 text-right font-mono ${(record.realized_slippage_bps ?? 0) <= 0 ? 'text-nofx-green' : 'text-nofx-red'}`}
                  >
                    {record.realized_slippage_bps?.toFixed(2) ?? '--'} bps
                  </td>
                  <td className="py-2 text-right font-mono text-nofx-text-main">
                    {record.partial_fill_ratio !== undefined
                      ? `${(record.partial_fill_ratio * 100).toFixed(0)}%`
                      : '--'}
                  </td>
