...
function structuralRiskReward(action: DecisionAction, isLong: boolean): number {
  if (action.protective_risk_reward && action.protective_risk_reward > 0) {
    return action.protective_risk_reward
  }
  if (!action.price || !action.take_profit) return 0
  const riskBasis =
    action.stop_loss_anchor && action.stop_loss_anchor > 0
      ? action.stop_loss_anchor
      : action.stop_loss
  if (!riskBasis) return 0
  const risk = isLong ? action.price - riskBasis : riskBasis - action.price
  const reward = isLong
...
function getTraceReason(trace: any): string {
  ...
  if (typeof trace?.primary?.reason === 'string' && trace.primary.reason)
    return trace.primary.reason
  if (typeof trace?.entry?.reason === 'string' && trace.entry.reason)
    return trace.entry.reason
  return ''
}
...
function healthText(
  ...
  if (rawReason.includes('limited warm-up'))
    return language === 'zh' ? '最低满足' : 'Minimum OK'
...
function marketDirectionReasons(
  marketContext: Record<string, any> | null,
  language: Language
): string[] {
  if (!marketContext) return []
  const metrics =
    marketContext.metrics && typeof marketContext.metrics === 'object'
  ...
  if (marketContext.funding_state) {
    reasons.push(
      language === 'zh'
        ? `资金费率 ${String(marketContext.funding_state)}`
        : `Funding ${String(marketContext.funding_state)}`
    )
  }
  if (typeof metrics.external_signal_score === 'number') {
    reasons.push(
      language === 'zh'
        ? `外部信号 ${metrics.external_signal_score.toFixed(2)}`
        : `External score ${metrics.external_signal_score.toFixed(2)}`
    )
  }
  const normalizedRV =
    metrics.average_realized_vol_20_15m_equivalent ??
    metrics.average_realized_vol_20
  if (typeof normalizedRV === 'number') {
    const atr =
      metrics.average_atr_percent_14_15m_equivalent ??
      metrics.average_atr_percent_14
...
function translateTraceReason(reason: string, language: Language): string {
  ...
  return reason
    ...
    .replace(/ or /g, ' 或 ')
    .replace(
      /confirmation timeframe evidence is unavailable/g,
      '确认周期证据不可用'
    )
    .replace(
      /(\\d+) confirmation timeframe\\(s\\) available/g,
      '$1 个确认周期可用'
    )
...
function FactorBreakdown({
  ...
  const components =
    trace.components && typeof trace.components === 'object'
      ? trace.components
      : {}
  const factorWeights =
    trace.factor_weights && typeof trace.factor_weights === 'object'
      ? trace.factor_weights
      : {}
  const factors =
  ...
  const availableFactors = Array.isArray(trace.available_factors)
    ? trace.available_factors
    : []
  const missingFactors = Array.isArray(trace.missing_factors)
    ? trace.missing_factors
    : []
  ...
  return (
    ...
    >
      <div
        className="mb-1 flex flex-wrap items-center gap-2 text-[10px]"
        style={{ color: '#848E9C' }}
      >
        <span>
          ...
          <span
            ...
            style={{ color: trace.eligible ? '#0ECB81' : '#F6465D' }}
          >
            {availableFactors.length}/
            {trace.selected_factor_count ?? factors.length}
          </span>
        </span>
        <span>
        ...
        <span>
          ...
          <span className="ml-1 font-mono" style={{ color: '#EAECEF' }}>
            {formatWeight(trace.available_weight)} /{' '}
            {formatWeight(trace.total_weight)}
          </span>
        </span>
        {missingFactors.length > 0 && (
          <span style={{ color: '#F6465D' }}>
            ...
            {missingFactors
              .map((f: string) => factorLabel(f, language))
              .join(', ')}
          </span>
        )}
      </div>
      <div className="grid grid-cols-2 gap-1">
        {factors.map((factor) => {
          ...
          return (
            <div
              key={`${trace.symbol || ''}-${trace.timeframe || ''}-${factor}`}
              className="flex items-center justify-between gap-2 text-[10px]"
            >
              <span style={{ color: available ? '#A7B0BC' : '#F6465D' }}>
                {factorLabel(factor, language)}
              </span>
              <span
                className="font-mono"
                style={{ color: available ? '#EAECEF' : '#F6465D' }}
              >
                {formatComponentValue(raw)}
                <span className="ml-1" style={{ color: '#848E9C' }}>
                  w {formatWeight(factorWeights[factor])}
                </span>
              </span>
            </div>
          )
    ...
    >
      ...
      {isOpen && action.stop_loss && action.take_profit && action.price && (
        ...
        >
          ...
          <div className="flex items-center gap-2">
            {(() => {
              const ratio = structuralRiskReward(action, isLong)
              const ratioColor =
...
export function DecisionCard({
  ...
  const parsedDecision = useMemo(
    () => parseDecisionJson(decision.decision_json),
    [decision.decision_json]
  )
  const snapshotSymbols = useMemo(
    () => getSnapshotSymbols(parsedDecision),
    [parsedDecision]
  )
  const setupEvaluations = useMemo(
    () => getArray(parsedDecision?.setup_evaluations),
    [parsedDecision]
  )
  const evidenceEvaluations = useMemo(
    () => getArray(parsedDecision?.evidence_evaluations),
    [parsedDecision]
  )
  const ruleEvaluations = useMemo(
    () => getArray(parsedDecision?.rule_evaluations),
    [parsedDecision]
  )
  const reviews = useMemo(
    () => getArray(parsedDecision?.reviews),
    [parsedDecision]
  )
  const riskRejected = useMemo(
    () => getArray(parsedDecision?.risk?.rejected),
    [parsedDecision]
  )
  const riskApproved = useMemo(
    () => getArray(parsedDecision?.risk?.approved),
    [parsedDecision]
  )
  const userDecisionSummary = useMemo(
    () =>
      getObject(parsedDecision?.user_decision_summary) ||
      getObject(decision.user_decision_summary),
    [parsedDecision, decision.user_decision_summary]
  )
  const inputAudit =
    parsedDecision?.input_audit &&
    typeof parsedDecision.input_audit === 'object'
      ? parsedDecision.input_audit
      : null
  const marketContext =
    parsedDecision?.market_context &&
    typeof parsedDecision.market_context === 'object'
      ? parsedDecision.market_context
      : null
  const signalCount = getArrayLength(parsedDecision?.signals)
  const candidateCount =
    decision.candidate_coins?.length || snapshotSymbols.length
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        ...
        >
          ...
          <div
            className="font-mono font-semibold"
            style={{ color: signalCount > 0 ? '#F0B90B' : '#A7B0BC' }}
          >
            {signalCount}
          </div>
        </div>
        ...
        >
          <div className="text-[10px]" style={{ color: '#848E9C' }}>
            {language === 'zh' ? '动作' : 'Actions'}
          </div>
          <div
            className="font-mono font-semibold"
            style={{ color: actionCount > 0 ? '#0ECB81' : '#A7B0BC' }}
          >
            {actionCount}
          </div>
        </div>
        ...
        >
          <div className="mb-2 flex items-center justify-between gap-3">
            ...
            <span
              ...
              style={{
                color: summaryStepColor(userDecisionSummary.status),
          ...
          {typeof userDecisionSummary.headline === 'string' &&
          ...
          {getArray(userDecisionSummary.steps).length > 0 && (
            <div className="space-y-2">
              {getArray(userDecisionSummary.steps)
                .slice(0, 4)
                  ...
                  >
                    <div className="mb-0.5 flex items-center gap-2 text-[11px]">
                      <span
                        ...
                        style={{ color: summaryStepColor(step?.status) }}
                      >
                        {String(step?.title || '-')}
                      </span>
          ...
          {getArray(userDecisionSummary.symbols).length > 0 && (
            <div className="mt-3 space-y-1">
              {getArray(userDecisionSummary.symbols)
                .slice(0, 3)
                .map((item: any) => (
                  <div
                    key={String(item?.symbol || item?.reason)}
                    className="text-[11px] leading-relaxed"
                    style={{ color: '#A7B0BC' }}
                  >
                    ...
                    <span
                      className="mx-1"
                      style={{ color: summaryStepColor(item?.decision) }}
                    >
        ...
        >
              ...
              >
                <div>
                  ...
                  <span
                    ...
                    style={{
                      color:
                        marketContext?.market_regime === 'risk_off'
                          ? '#F6465D'
                          : '#EAECEF',
                    }}
                  >
                ...
                <div>
                  ...
                  <span
                    ...
                    style={{
                      color:
                        marketContext?.direction_bias === 'bearish'
                          ? '#F6465D'
                          : marketContext?.direction_bias === 'bullish'
                            ? '#0ECB81'
                            : '#EAECEF',
                    }}
                  >
                  ...
                  <span
                    ...
                    style={{
                      color:
                        marketContext?.volatility_regime === 'high_volatility'
                          ? '#F0B90B'
                          : '#EAECEF',
                    }}
                  >
                ...
                <div>
                  ...
                  <span
                    ...
                    style={{
                      color:
                        marketContext?.btc_trend === 'bearish'
                          ? '#F6465D'
                          : '#EAECEF',
                    }}
                  >
                  ...
                  <span
                    ...
                    style={{
                      color:
                        marketContext?.eth_trend === 'bearish'
                          ? '#F6465D'
                          : '#EAECEF',
                    }}
                  >
                ...
                {Array.isArray(marketContext?.risk_flags) &&
                  marketContext.risk_flags.length > 0 && (
                    <div className="col-span-2" style={{ color: '#F0B90B' }}>
                      {language === 'zh' ? '风险标记' : 'Risk flags'}:{' '}
                      {marketContext.risk_flags
                        .map((flag: unknown) => riskFlagLabel(flag, language))
                        .join(', ')}
                    </div>
                  )}
                <div className="col-span-2">
                  ...
                  <span className="ml-1">
                    {marketDirectionReasons(marketContext, language).join(
                      '；'
                    ) || '-'}
                  </span>
                </div>
                {typeof marketContext?.context_summary === 'string' &&
            ...
            >
              ...
              >
                <div>
                  ...
                  <span className="ml-1 font-mono" style={{ color: '#EAECEF' }}>
                    {String(inputAudit?.klines?.market_data_source || '-')}
                  </span>
                </div>
                <div>
                  {language === 'zh' ? '请求周期' : 'Timeframes'}:
                  <span className="ml-1 font-mono" style={{ color: '#EAECEF' }}>
                    {Array.isArray(inputAudit?.klines?.timeframes)
                      ? inputAudit.klines.timeframes.join(', ')
                      : '-'}
                  </span>
                </div>
                <div>
                  ...
                  :
                  <span className="ml-1 font-mono" style={{ color: '#EAECEF' }}>
                    {String(inputAudit?.klines?.primary_timeframe || '-')} /{' '}
                    {String(inputAudit?.klines?.entry_timeframe || '-')} /{' '}
                    {Array.isArray(inputAudit?.klines?.confirmations)
                      ? inputAudit.klines.confirmations.join(', ') || '-'
                      : '-'}
                  </span>
                </div>
                <div>
                  ...
                  <span className="ml-1 font-mono" style={{ color: '#EAECEF' }}>
                    {String(inputAudit?.klines?.compute_lookback || '-')}
                  </span>
                </div>
                <div>
                  {language === 'zh' ? '展示 K 线' : 'Display Bars'}:
                  <span className="ml-1 font-mono" style={{ color: '#EAECEF' }}>
                    {String(inputAudit?.klines?.display_count || '-')}
                  </span>
                </div>
                <div>
                  {language === 'zh' ? '最低计算需求' : 'Required Bars'}:
                  <span className="ml-1 font-mono" style={{ color: '#EAECEF' }}>
                    {String(inputAudit?.klines?.required_lookback || '-')}
                  </span>
                </div>
                <div>
                  {language === 'zh' ? '稳定建议' : 'Warm-up Target'}:
                  <span className="ml-1 font-mono" style={{ color: '#EAECEF' }}>
                    {String(inputAudit?.klines?.warmup_target || '-')}
                  </span>
                </div>
                {Array.isArray(inputAudit?.klines?.unused_timeframes) &&
                  inputAudit.klines.unused_timeframes.length > 0 && (
                    <div className="col-span-2" style={{ color: '#F0B90B' }}>
                      {language === 'zh'
                        ? '已拉取但未参与当前评分'
                        : 'Fetched but not used by current scoring'}
                      :
                      <span className="ml-1 font-mono">
                        {inputAudit.klines.unused_timeframes.join(', ')}
                      </span>
                    </div>
                  )}
                <div>
                  ...
                  <span
                    className="ml-1"
                    style={{
                      color: inputAudit?.external_data?.oi_ranking_available
                        ? '#0ECB81'
                        : '#F6465D',
                    }}
                  >
                    {statusText(
                      inputAudit?.external_data?.statuses?.oi_ranking,
                      language
                    ) ||
                      boolText(
                        inputAudit?.external_data?.oi_ranking_available,
                        language
                      )}
                  </span>
                </div>
                <div>
                  ...
                  <span
                    className="ml-1"
                    style={{
                      color: inputAudit?.external_data?.netflow_available
                        ? '#0ECB81'
                        : '#F6465D',
                    }}
                  >
                    {statusText(
                      inputAudit?.external_data?.statuses?.netflow,
                      language
                    ) ||
                      boolText(
                        inputAudit?.external_data?.netflow_available,
                        language
                      )}
                  </span>
                </div>
                <div>
                  ...
                  <span
                    className="ml-1"
                    style={{
                      color: inputAudit?.external_data?.price_ranking_available
                        ? '#0ECB81'
                        : '#F6465D',
                    }}
                  >
                    {statusText(
                      inputAudit?.external_data?.statuses?.price_ranking,
                      language
                    ) ||
                      boolText(
                        inputAudit?.external_data?.price_ranking_available,
                        language
                      )}
                  </span>
                </div>
                <div>
                  ...
                  <span
                    ...
                    style={{
                      color:
                        (inputAudit?.external_data?.quant_symbols || 0) > 0
                          ? '#0ECB81'
                          : '#F6465D',
                    }}
                  >
                    ...
                    <span className="ml-1" style={{ color: '#A7B0BC' }}>
                      {statusText(
                        inputAudit?.external_data?.statuses?.quant,
                        language
                      )}
                    </span>
                  </span>
              ...
              {Array.isArray(inputAudit?.external_data?.data_fetch_errors) &&
                inputAudit.external_data.data_fetch_errors.length > 0 && (
                  <div
                  ...
                  >
                    {language === 'zh' ? '数据错误' : 'Data errors'}:{' '}
                    {inputAudit.external_data.data_fetch_errors
                      .slice(0, 2)
                      .join('; ')}
                  </div>
                )}
              {inputAudit?.symbols &&
                typeof inputAudit.symbols === 'object' && (
                  <div className="mt-2 space-y-1">
                    {Object.entries(inputAudit.symbols)
                      .slice(0, 6)
                      .map(([symbol, audit]: [string, any]) => (
                        <div
                          key={`${symbol}-input-audit`}
                        ...
                        >
                          ...
                          <div
                            className="mt-1 grid grid-cols-1 gap-1 text-[10px]"
                            style={{ color: '#848E9C' }}
                          >
                            {Object.entries(audit?.timeframes || {}).map(
                              ([tf, tfAudit]: [string, any]) => (
                                <div key={`${symbol}-${tf}`}>
                                  <span
                                    className="font-mono"
                                    style={{ color: '#A7B0BC' }}
                                  ...
                                  <span className="ml-2">
                                    {language === 'zh'
                                      ? '实际/计算'
                                      : 'display/compute'}{' '}
                                    {String(tfAudit?.display_bars ?? '-')} /{' '}
                                    {String(tfAudit?.compute_bars ?? '-')}
                                  </span>
                                  <span className="ml-2">
                                    {language === 'zh'
                                      ? '需/余量'
                                      : 'required/warmup'}{' '}
                                    {String(tfAudit?.required_lookback ?? '-')}{' '}
                                    / {String(tfAudit?.warmup_bars ?? '-')}
                                  </span>
                                  <span
                                    className="ml-2"
                                    style={{
                                      color: healthColor(
                                        tfAudit?.calculation_healthy,
                                        tfAudit?.health_reason
                                      ),
                                    }}
                                  >
                                    {healthText(
                                  ...
                                  <span className="ml-2">
                                    ...
                                    {latestTimeText(tfAudit?.latest_time)}
                                  </span>
                                  <span className="ml-2">
                                    close{' '}
                                    {formatPrice(
                                      typeof tfAudit?.latest_close === 'number'
                                        ? tfAudit.latest_close
                                        : undefined
                                    )}
                                  </span>
                                </div>
            ...
            <div className="space-y-2 mb-3">
              ...
              {ruleEvaluations.slice(0, 12).map((trace: any, index: number) => (
                ...
                >
                  ...
                  <div className="mt-2 space-y-1">
                    {Array.isArray(trace?.conditions) &&
                      trace.conditions.map((c: any, ci: number) => (
                        <div
                          key={ci}
                        ...
                        >
                          ...
                          <span style={{ color: '#A7B0BC' }}>
                            {c?.left_available &&
                            typeof c?.left_value === 'number'
                              ? `(${c.left_value.toFixed(2)})`
                              : '(n/a)'}
                          </span>
                          ...
                          <span style={{ color: '#A7B0BC' }}>
                            {c?.right_available &&
                            typeof c?.right_value === 'number'
                              ? `(${c.right_value.toFixed(2)})`
                              : ''}
                          </span>
                        </div>
                      ))}
          ...
          {(reviews.length > 0 ||
            ...
            <div className="space-y-2 mb-3">
              ...
              >
                {language === 'zh'
                  ? '信号复核 / 风控门（为什么被拒 / 放行）'
                  : 'Review / Risk Gate (why rejected / approved)'}
              </div>
              <div
              ...
              >
                ...
                <span>
                  ...
                  <span className="font-mono" style={{ color: '#F6465D' }}>
                    {riskRejected.length}
                  </span>
                </span>
                <span>
                  {language === 'zh' ? '复核' : 'Reviews'}:{' '}
                  <span className="font-mono" style={{ color: '#EAECEF' }}>
                    {reviews.length}
                  </span>
                </span>
            ...
            {setupEvaluations.slice(0, 6).map((trace, index) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div>
                      {language === 'zh' ? '主周期' : 'Primary'}{' '}
                      {String(timeframes?.primary || '-')}
                      <span
                        className="ml-1 font-mono"
                        style={{ color: '#EAECEF' }}
                      >
                        {formatScore(primary?.score)}
                      </span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      {Array.isArray(timeframes?.confirmations) &&
                      timeframes.confirmations.length > 0
                  ...
                  <div className="mt-2 grid grid-cols-1 gap-2">
                    ...
                    <div>
                      ...
                      >
                        {language === 'zh'
                          ? '入场周期因子拆分'
                          : 'Entry factor breakdown'}
                      </div>
                      <FactorBreakdown trace={entry} language={language} />
                    </div>
                    {Array.isArray(trace?.confirmations) &&
                      trace.confirmations.length > 0 && (
                        <div>
                          <div
                            className="text-[10px] font-semibold"
                            style={{ color: '#A7B0BC' }}
                          >
                            {language === 'zh'
                              ? '确认周期因子拆分'
                              : 'Confirmation factor breakdown'}
                          </div>
                          <div className="space-y-1">
                            {trace.confirmations.map(
                              (confirm: any, confirmIndex: number) => (
                                <div
                                  key={`${String(trace?.symbol || index)}-confirm-${confirmIndex}`}
                                >
                                  <div
                                    className="mt-1 text-[10px]"
                                    style={{ color: '#848E9C' }}
          ...
          {setupEvaluations.length === 0 && evidenceEvaluations.length > 0 && (
            <div className="text-[11px]" style={{ color: '#A7B0BC' }}>
              {language === 'zh'
                ? '本轮只有证据过滤评估，没有 setup 评估。'
                : 'This cycle has evidence-filter evaluations but no setup evaluation.'}
            </div>
          )}
        </div>
      )}
