...
export interface RiskControlConfig {
  ...
  entry_risk_guard?: {
    ...
    // Override the global mode ONLY for the extended-TP reason.
    // Empty string = inherit global mode (default).
    take_profit_guard_mode?: '' | 'hard_block' | 'warn_reduce'
    // Low R:R protection: configurable soft/hard floor.
    // When enabled, R/R between soft floor and MinRR triggers warn_reduce
  ...
  consecutive_loss_brake?: {
    ...
    scope?: 'global' | 'direction' | 'symbol_side' // Counting scope (default: global)
  }

  // Trend end watch: when the trailing N closed trades for a (direction |
  // symbol_side) all missed take-profit, pause new entries for that scope.
  trend_end_watch?: {
    enabled: boolean // Enable (default: false, opt-in)
    misses: number // Consecutive non-TP closes that trigger cooldown (default: 3)
    cool_down_cycles: number // Block entries for K cycles (default: 3)
    scope?: 'direction' | 'symbol_side' // Counting scope (default: direction)
  }
